...
function getMoonIcon() {
    return `
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="28" height="28">
            <path fill="currentColor" d="M12 3a9 9 0 109 9c0-.46-.04-.92-.1-1.36a5.389 5.389 0 01-4.4 2.26a5.403 5.403 0 01-3.14-9.8c-.44-.06-.9-.1-1.36-.1z"/>
        </svg>
    `;
}
...
function setupMobileMenu() {
    if (!DOM.mobileMenuToggle || !DOM.navList) {
...
function setupLoadingScreen() {
    ...
    // Fallback: hide loading screen after maximum delay
    setTimeout(() => {
        if (!loadingScreen.classList.contains('hidden')) {
            loadingScreen.classList.add('hidden');
            console.log('Loading screen hidden (fallback)');
        }
    }, CONFIG.LOADING_DELAY + 2000);
    
    console.log('Loading screen setup complete');
}
...
function setupScrollProgress() {
    ...
    console.log('Scroll progress setup complete');
}
...
function setupCounterAnimations() {
    ...
    if (STATE.counters.length > 0) {
        console.log(`Found ${STATE.counters.length} counter elements`);
    }
}
...
function checkCounterVisibility() {
    if (STATE.countersAnimated || !STATE.counters) return;
...
function setupParallaxEffects() {
    console.log('Parallax effects setup complete');
}
...
function initializeAOS() {
    console.log('AOS animations setup complete');
}
